...
local function get_term_config(type)
	local config = {}
...
local function new_terminal(type)
	...
	local config = get_term_config(type)

	vim.api.nvim_create_autocmd("VimResized", {
		group = vim.api.nvim_create_augroup("terminal-resize", { clear = false }),
		callback = function()
			config = get_term_config(type)
		end,
	})
	...
	return function()
		...
		if not buf_valid then
			vim.wo[term_win].winhighlight = "NormalFloat:Normal"
